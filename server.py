...
def mqtt_on_message(client: mqtt.Client, userdata: dict, msg: mqtt.MQTTMessage):
    ...
    if PEER_KEY != public_key:
        ...
        ROOT_KEY, CHAIN_KEY = ratchet.kdf_rk(dh_parameters.ROOT_KEY, dh_output)
        PEER_KEY = public_key

    if len(msg.payload) == len(public_key):
        mqtt_client.publish('tor.in', KEY_PAIR.public_key().public_bytes(serialization.Encoding.PEM, serialization.PublicFormat.SubjectPublicKeyInfo).strip())
        return
...
while True:
    ...
    mqtt_client.publish('tor.in', mqtt_message)
    time.sleep(0.25)